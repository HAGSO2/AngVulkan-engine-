...
void create(vulkan_context* context, u32 width, u32 height, vulkan_swapchain* swapchain) {
    ...
    if (swapchain->images.empty()) {
        swapchain->images = vector<VkImage>(swapchain->image_count);
    }
    if (swapchain->views.empty()) {
        swapchain->views = vector<VkImageView>(swapchain->image_count);
    }
    VK_CHECK(vkGetSwapchainImagesKHR(context->device.logical_device, swapchain->handle, &swapchain->image_count, swapchain->images.data()));
