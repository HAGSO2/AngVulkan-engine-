...
b8 select_physical_device(vulkan_context* context, vulkan_options* options);
...
b8 vulkan_device_create(vulkan_context* context, vulkan_options* options) {
    if (!select_physical_device(context, options)) {
        return FALSE;
    }
    ...
    VkDeviceQueueCreateInfo* queue_create_infos = new VkDeviceQueueCreateInfo[index_count];
...
void vulkan_device_destroy(vulkan_context* context) {
    ...
    memset(
        &context->device.swapchain_support.capabilities, 0,
...
void vulkan_device_query_swapchain_support(
    ...
    if (out_support_info->format_count != 0) {
        if (out_support_info->formats.empty()) {
            out_support_info->formats = vector<VkSurfaceFormatKHR>(out_support_info->format_count);
        }
        VK_CHECK(vkGetPhysicalDeviceSurfaceFormatsKHR(
            physical_device,
            surface,
    ...
    if (out_support_info->present_mode_count != 0) {
        if (out_support_info->present_modes.empty()) {
            out_support_info->present_modes = vector<VkPresentModeKHR>(out_support_info->present_mode_count);
        }
        VK_CHECK(vkGetPhysicalDeviceSurfacePresentModesKHR(
            physical_device,
            surface,
...
b8 select_physical_device(vulkan_context* context, vulkan_options* options) {
    ...
    for (u32 i = 0; i < physical_device_count; ++i) {
        ...
        requirements.sampler_anisotropy = options->sampler_anisotropy;
        requirements.discrete_gpu = options->discrete_gpu;
        requirements.device_extension_names = vector<const char*>();
...
b8 physical_device_meets_requirements(
    ...
    if (
        ...
        if (!requirements->device_extension_names.empty()) {
            ...
            vector<VkExtensionProperties> available_extensions = vector<VkExtensionProperties>();
            ...
            if (available_extension_count != 0) {
                available_extensions = vector<VkExtensionProperties>(available_extension_count);
                VK_CHECK(vkEnumerateDeviceExtensionProperties(
                    device,
                    0,
                    &available_extension_count,
